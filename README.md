...
- 재고(ProductStock)
  - [ ] 재고는 각 상품(Product)과 그에 해당하는 재고 수량(Stock)을 갖는다.
    - [ ] 재고 수량은 재고의 갯수를 나타내는 필드를 갖는다.
    - [ ] 재고 수량은 생성될 때, 0 이상이여야 한다.
  - [ ] 상품을 구매하면 재고 수량은 구매한 만큼 감소한다.
    - [ ] 구매하는 수가 재고 수량보다 많다면 예외를 발생한다.
...
- 프로모션(Promotion)
  - [ ] 프로모션 이름과 기간을 갖는다.
  - [ ] N개 구매 시 1개 무료 증정(Buy N Get 1 Free)의 형태를 갖는다.
  - [ ] 프로모션 재고를 갖는다.
